...
namespace HospitalNet.Backend.BusinessLogic
{
    ...
    public class AppointmentManager
    {
        ...
        /// Calls sp_GetAvailableTimeSlots stored procedure.
        /// </summary>
        /// <param name="doctorId">The ID of the doctor.</param>
        /// <param name="date">The date to check for availability.</param>
        /// <returns>List of DateTime objects representing available time slots.</returns>
        public List<DateTime> GetAvailableTimeSlots(int doctorId, DateTime date)
        {
            try
            {
                ...
                {
                    DatabaseHelper.CreateInputParameter("@DoctorID", doctorId),
                    DatabaseHelper.CreateInputParameter("@Date", date.Date)
                };

                var timeSlots = _dbHelper.ExecuteReader<DateTime>(
                    "sp_GetAvailableTimeSlots",
                    reader => reader.GetDateTime(reader.GetOrdinal("AvailableTime")),
                    parameters);
            ...
            {
                throw new Exception($"Database error while retrieving available time slots: {sqlEx.Message}", sqlEx);
            }
            catch (Exception ex)
            {
                throw new Exception($"Error retrieving available time slots: {ex.Message}", ex);
            }
        }
    }
}
